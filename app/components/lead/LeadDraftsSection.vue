...
// ----- Génération : déléguée à DraftGeneratorModal (ouverte via son API exposée) -----
const generatorModal = ref<{ open: (type?: DraftType) => void } | null>(null)
async function onGenerationRequested(): Promise<void> {
  await refreshDrafts()
  startPolling()
  emit('activity')
}
...
onMounted(() => {
  if (route.query.draft === 'follow-up' && props.canGenerate) {
    generatorModal.value?.open('FOLLOW_UP_EMAIL')
  }
})
...
<template>
  <section class="mt-10">
    ...
    <!-- Modale « Générer un brouillon » (ouverte via son API exposée) -->
    <DraftGeneratorModal
      ref="generatorModal"
      :lead-id="leadId"
      :language-pair="languagePair"
      @requested="onGenerationRequested"
    />
