...
import { expectToast, login, waitForHydration, watchConsole } from './helpers'
...
test('boîte email : connexion OAuth (factice) depuis les réglages, état, déconnexion', async ({ page }) => {
  ...
  if (await connectedBadge.isVisible()) {
    ...
    await expectToast(page, /boîte déconnectée|mailbox disconnected/i)
  }
  ...
  await expectToast(page, /boîte connectée|mailbox connected/i)
